...
GROUP BY r.rol_id, h.horario_inicio, h.horario_fin
  ...
  @Get("/roles/:id")
  @Response<any>(200, "Rol encontrado", {
    ok: true,
    msg: "Rol encontrado exitosamente",
    status: 200,
    data: [],
  })
  @Response<NotFoundItems>(404, "Rol no encontrado", {
    ok: false,
    msg: "El rol solicitado no fue encontrado",
    status: 404,
  })
  @Response<InternalServerError>(500, "Internal Server Error", {
    ok: false,
    msg: "Error interno del sistema al obtener el rol",
    error: {},
    status: 500,
  })
  public async obtenerRolPorId(
    @Path() id: number
  ): Promise<any | NotFoundItems | InternalServerError> {
    try {
      const query = "SELECT * FROM roles WHERE rol_id = ?";
      const [rol] = await execute(query, [id]);
